...
# Change outcome variables from numbers to letters to accomodate RFE algorithm requirements
...
```{r Embedded method and summary, Question 1}
# random forest
...
# Overall, these methods did produce similar results. Random forest selected Mg, Al, RI, Ca and Na to be the top 5 most important variables, while recursive feature selection chose the top five variables to be Mg, Al, Na, K, and Ca. Only the variables RI and K did not overlap; however, K was the sixth most important variable according to the random forest model. 
```

```{r Genetic algorithm, Question 2}
# Genetic algorithm

# Set training and test sets
set.seed(10)
train_size_gen <- floor(0.75 * nrow(Glass))
train_pos_gen <- sample(seq_len(nrow(Glass)), size = train_size)

train_classification_gen <- Glass[train_pos_gen, ]
test_classification_gen <- Glass[-train_pos_gen, ]

# define control 
control <- gafsControl(functions = caretGA, genParallel=TRUE, allowParallel=TRUE,)

# genetic algorithm
results_gafs <- gafs(train_classification_gen[,1:9], train_classification_gen[,10], iters = 5, gafsControl = control, method = "svmRadial")

results_gafs
results_gafs$variables
